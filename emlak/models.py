...
class Category(MPTTModel):
    ...
    status = models.CharField(max_length=10, choices=STATUS)
    slug = models.SlugField(null=False, unique=True)
    parent = TreeForeignKey('self', blank=True, null=True, related_name='children', on_delete=models.CASCADE) #kendi kendine refere eder, boş kabul edilir,silme işleminde silenini tüm alt bağlarıyla beraber siler.  create_at = models.DateTimeField(auto_now_add=True)
    update_at = models.DateTimeField(auto_now=True)
